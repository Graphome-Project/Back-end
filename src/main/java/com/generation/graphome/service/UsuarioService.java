...
public class UsuarioService {
	...
	public Optional <Usuario> cadastrarUsuario(Usuario usuario){
		...
		if (calcularIdade(usuario.getDataNascimento())<18)
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Sua idade é menor que 18",null);
	...
	public Optional <Usuario> atualizarUsuario (Usuario usuario){
		...
		if ((buscaUsuario.isPresent())&&(buscaUsuario.get().getId()!=usuario.getId()))
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"Usuario já Existente",null);
	...
	public Optional <UsuarioLogin> autenticarUsuario (Optional<UsuarioLogin> usuarioLogin){
		...
		if (buscaUsuario.isPresent()) {
			if (compararSenha(usuarioLogin.get().getSenha(),buscaUsuario.get().getSenha())) {
